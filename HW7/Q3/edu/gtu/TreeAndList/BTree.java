...
public class BTree<E extends Comparable<E>>  implements SearchTree<E> {
    ...
    /**
     * Return the the appropriate position of given item on given array.
     * @param item item
     * @param data array
    ...
    private int binarySearch(E item, E[] data , int index , int size) {
        ...
        if( index < size ) {
            ...
            if( result == 0 ) {
                ...
                return binarySearch( item , data , index , mid );
            } else {
     ...
     * <p> post : node.data[index-1] &lt; item %lt; node.data[index];</p>
     * <p> pre : node.data[index] == item and old values are moved right one</p>
